...
public class PaintAreaView extends View {
    ...
    private ArrayList<PaintPath> undonePaths = new ArrayList<PaintPath>();
    ...
    public PaintAreaView(Context context) {
        ...
        pathPaint.setStrokeWidth(PAINT_RADIUS * 5);
    ...
    {
        this.pathPaint.setStrokeWidth(PAINT_RADIUS*size);
    }

    public void change_background_color(int color)
    {
    ...
    {
        return (int)(this.pathPaint.getStrokeWidth()/PAINT_RADIUS);
    }
    @Override
    ...
    private void onTouchUp() {
        ...
        curPaintPath = null;
        ((MainActivity)getContext()).show();
    }
    public void onClickUndo () {
        Log.d("Path Size",paths.size()+"");
        if (paths.size()>0)
        {
            undonePaths.add(paths.get(paths.size()-1));
            paths.remove(paths.size()-1);
            Log.d("Path New Size",paths.size()+"");
//            invalidate();
            canvas.drawColor(BACKGROUND_COLOR);
